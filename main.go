...
func clickElement(page *rod.Page, el *rod.Element, button string, force bool) {
	if button == "left" {
		if force {
	...
	page.Mouse.MustDown(proto.InputMouseButton(button))
	page.Mouse.MustUp(proto.InputMouseButton(button))
}
...
func cmdClick(args []string) {
	...
	for i := 0; i < len(args); i++ {
		...
		} else if args[i] == "--button" && i+1 < len(args) {
			i++
			button = args[i]
			if button != "left" && button != "right" && button != "middle" {
				fmt.Fprintf(os.Stderr, "ERROR: --button must be left, right, or middle\n")
				os.Exit(1)
			}
		} else {
	...
	if args[0] == "--text" && len(args) > 1 {
		...
		clickElement(page, el, button, force)
		...
		page.Mouse.MustDown(proto.InputMouseButton(button))
		page.Mouse.MustUp(proto.InputMouseButton(button))
		fmt.Printf("Clicked (%s, %s) [%s]\n", args[1], args[2], button)
	} else {
...
func cmdType(args []string) {
	if len(args) == 0 { fmt.Fprintln(os.Stderr, "Usage: wdb type <text>"); os.Exit(1) }
	browser := connectBrowser()
	page := getPage(browser)
...
func cmdEval(args []string) {
	if len(args) == 0 { fmt.Fprintln(os.Stderr, "Usage: wdb eval <js>"); os.Exit(1) }
	browser := connectBrowser()
	page := getPage(browser)
	frame := getFrame(page)
...
func cmdWait(args []string) {
	if len(args) == 0 { fmt.Fprintln(os.Stderr, "Usage: wdb wait <selector|ms>"); os.Exit(1) }
	browser := connectBrowser()
	page := getPage(browser)
	frame := getFrame(page)
...
const manifestJSON = `{"schema":"actuators-manifest/v1","name":"wdb","version":"0.1.0","category":"device-control","deps":[{"name":"Google Chrome","kind":"system","min":null,"rationale":"CDP target via go-rod"}],"provides":["wdb-browser-v1"],"consumes":[],"configs":[]}`
...
func main() {
	...
	if len(os.Args) < 2 {
		...
		fmt.Println("  click <sel>         Click (--text/--xy/--force/--within/--nth/--button)")
